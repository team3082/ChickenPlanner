...
public class BezierSpline {
    private final ArrayList<BezierCurve> curveList;
     ...
     */
    public BezierSpline(BezierCurve... curves) {
        if (curves == null) {
    ...
    public BezierSpline(BezierSpline other) {
        ...
        curveList = new ArrayList<>();
        for (BezierCurve curve : other.curveList) {
            curveList.add(new BezierCurve(curve)); // Assuming CubicBezierCurve has a copy constructor
        }
    }
    ...
    public double findNearestT(Vector2 point) {
        ...
        for (int curveIndex = 0; curveIndex < curveList.size(); curveIndex++) {
            BezierCurve cubicBezierCurve = curveList.get(curveIndex);
     ...
     */
    public void addCurve(BezierCurve curve, int index) {
        if (curve == null) {
        ...
        //Checks if desired location is at the end of the spline
        if (index == curveList.size()){
            curveList.add(curve);
        } else {
            curveList.add(index, curve);
        }
        //If there is a curve to the right, sets point four of the new curve to point one of the curve on the right
        if(index < curveList.size()){
            BezierCurve rightCurve = curveList.get(index+1);
            if(!curve.getControlPointFour().equals(rightCurve.getControlPointOne())){
                curve.setControlPointFour(rightCurve.getControlPointOne());
            }
        }
        //If there is a curve to the left, sets point one of the new curve to point four of the curve on the left
        if(index > 0){
            BezierCurve leftCurve = curveList.get(index-1);
    ...
    public void removeCurve(int curveIndex) {
        ...
        if(curveIndex == curveList.size()){
            ...
            BezierCurve curveLeft = curveList.remove(curveIndex-1);
            BezierCurve curveRight = curveList.remove(curveIndex-1);
            BezierCurve mergedCurve = new BezierCurve(
     ...
     */
    public ArrayList<BezierCurve> getCurveArray() {
        return curveList;
    }
}
